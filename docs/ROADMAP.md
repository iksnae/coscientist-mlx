...
feature PRs rather than numbered milestones. Going forward, substantial
work runs through the milestone loop (`milestone-planner` →
`milestone-grinder`); the next numbered milestone is **M6**.
...
These are candidate directions, not committed scope. `milestone-planner`
turns a chosen theme + operator intention into grindable drafts.

- **Optimization (orig. M7).** Batched inference, prefix/KV cache reuse,
  quant tiers — performance levers now that parity holds.
